...
async function makeLogoTransparent(src) {
  try {
    ...
    for (let y=0;y<H;y++) for (let x=0;x<W;x++) if (d.data[(y*W+x)*4+3]>8) { if(x<x0)x0=x; if(x>x1)x1=x; if(y<y0)y0=y; if(y>y1)y1=y; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  makeLogoTransparent('ethos-logo.png').then(url => {
    document.querySelectorAll('img.logo-img').forEach(el => {
      if (url) el.src = url;
